...
class BaseRecycler:
  ...
  def log_historical_dead_neuron_overlapping(self, intermediates, update_step):
    ...
    if self.historical_dormant_mask is None:
      ...
      # print(neuron_score_dict.keys())
      # ['critic0/dense0_act/__call__', 'critic0/dense1_act/__call__', 'critic1/dense0_act/__call__', 'critic1/dense1_act/__call__']
      # ['conv0_act/__call__', 'conv1_act/__call__', 'conv2_act/__call__', 'conv3_act/__call__']
      # import time
      # time.sleep(10)
      # ['/dense-1_layernorm_tanh_act/__call__', 'CriticHead/critic0/dense0_act/__call__', 'CriticHead/critic0/dense1_act/__call__', 'CriticHead/critic1/dense0_act/__call__', 'CriticHead/critic1/dense1_act/__call__', 'SharedEncoder/conv0_act/__call__', 'SharedEncoder/conv1_act/__call__', 'SharedEncoder/conv2_act/__call__', 'SharedEncoder/conv3_act/__call__']
...
class NeuronRecycler(BaseRecycler):
  ...
  def __init__(
    ...
    # outgoing weights.
    if 'conv0' in all_layers_names:
      self.next_layers_for_enc = {}
      for current_layer, next_layer in zip(
          all_layers_names[:-1], all_layers_names[1:]
      ):
        if 'tanh' in current_layer:
          break
        self.next_layers_for_enc[current_layer] = next_layer
      # print(all_layers_names)['conv0', 'conv1', 'conv2', 'conv3', 'dense-1_layernorm_tanh', 'critic0/dense0', 'critic0/dense1', 'critic1/dense0', 'critic1/dense1']
      self.next_layers_for_critic0 = {}
      for current_layer, next_layer in zip(
          all_layers_names[4:-3], all_layers_names[5:-2]
      ):
        self.next_layers_for_critic0[current_layer] = next_layer
      all_layers_names_ = all_layers_names.copy()
      all_layers_names_.pop(5)
      all_layers_names_.pop(5)
      self.next_layers_for_critic1 = {}
      for current_layer, next_layer in zip(
          all_layers_names_[4:-1], all_layers_names_[5:]
      ):
        self.next_layers_for_critic1[current_layer] = next_layer
    else: # only redo critic heads
      self.next_layers_for_critic0 = {}
      for current_layer, next_layer in zip(
          all_layers_names[0:1], all_layers_names[1:2]
      ):
        self.next_layers_for_critic0[current_layer] = next_layer
      self.next_layers_for_critic1 = {}
      for current_layer, next_layer in zip(
          all_layers_names[2:3], all_layers_names[3:4]
      ):
        self.next_layers_for_critic1[current_layer] = next_layer
  ...
  def recycle_dead_neurons(self, intermedieates, params, key, opt_state_set):
    ...
    # print(activations_score_dict.keys())    
    ...
    # print(4343, outgoing_mask_dict.keys())
    # print(2233, incoming_mask_dict.keys())
    # import time
    # time.sleep(2)
    ...
    # print(incoming_mask[''].keys())#['', 'CriticHead', 'LayerNorm_0', 'SharedEncoder']
    # print(434, incoming_mask.keys())
    # print(949, incoming_mask_dict.keys())
    # print(394,param_dict.keys())
    # import time
    # time.sleep(2)
    ...
    # print(4223, outgoing_mask.keys())
    # print(934343, outgoing_mask_dict.keys())
    ...
    if self.init_method_outgoing == 'random':
      ...
      #   print(1, k1, type(params[k1]))
      #   print(11, params[k1].keys())
      #   for k2 in params[k1].keys():
      #     print(2, k2, type(params[k2]))
      #     print(22, params[k2].keys())
      # for k1 in param_dict.keys():
      #   print(11, k1, param_dict[k1].shape)
      # for k1 in outgoing_mask_dict.keys():
      #   if outgoing_mask_dict[k1] is not None:
      #     print(k1)
      #     print(outgoing_mask_dict[k1].shape)
    ...
    new_opt_state_set = []
    # print(incoming_mask.keys())
    # print(incoming_mask_dict.keys())
    # dict_keys(['/dense-1_layernorm_tanh/bias', '/dense-1_layernorm_tanh/kernel', 'CriticHead/critic0/dense0/bias', 
    # 'CriticHead/critic0/dense0/kernel', 'CriticHead/critic0/dense1/bias', 'CriticHead/critic0/dense1/kernel', 
    # 'CriticHead/critic0/final/bias', 'CriticHead/critic0/final/kernel', 'CriticHead/critic1/dense0/bias', 
    # 'CriticHead/critic1/dense0/kernel', 'CriticHead/critic1/dense1/bias', 'CriticHead/critic1/dense1/kernel', 
    # 'CriticHead/critic1/final/bias', 'CriticHead/critic1/final/kernel', 'LayerNorm_0/bias', 'LayerNorm_0/scale', 
    # 'SharedEncoder/conv0/bias', 'SharedEncoder/conv0/kernel', 'SharedEncoder/conv1/bias', 
    # 'SharedEncoder/conv1/kernel', 'SharedEncoder/conv2/bias', 'SharedEncoder/conv2/kernel', 
    # 'SharedEncoder/conv3/bias', 'SharedEncoder/conv3/kernel'])
    incoming_mask_enc = incoming_mask['SharedEncoder']
    incoming_mask_head = {k: incoming_mask[k] for k in incoming_mask.keys() if 'Encoder' not in k}
    incoming_mask_head['/dense-1_layernorm_tanh'] = incoming_mask_head['']['dense-1_layernorm_tanh']
    incoming_mask_head.pop('')
    incoming_masks = [flax.core.FrozenDict(incoming_mask_enc), flax.core.FrozenDict(incoming_mask_head)]
    outgoing_mask_enc = outgoing_mask['SharedEncoder']
    outgoing_mask_head = {k: outgoing_mask[k] for k in outgoing_mask.keys() if 'Encoder' not in k}
    outgoing_mask_head['/dense-1_layernorm_tanh'] = outgoing_mask_head['']['dense-1_layernorm_tanh']
    outgoing_mask_head.pop('')
    outgoing_masks = [flax.core.FrozenDict(outgoing_mask_enc), flax.core.FrozenDict(outgoing_mask_head)]

    for i, opt_state in enumerate(opt_state_set):
      new_mu = reset_momentum_fn(opt_state[0][1], incoming_masks[i])
      new_mu = reset_momentum_fn(new_mu, outgoing_masks[i])
      new_nu = reset_momentum_fn(opt_state[0][2], incoming_masks[i])
      new_nu = reset_momentum_fn(new_nu, outgoing_masks[i])
      opt_state_list = list(opt_state)
      opt_state_list[0] = optax.ScaleByAdamState(
          opt_state[0].count, mu=new_mu, nu=new_nu
      )
      # new_opt_state = tuple(opt_state_list)
      new_opt_state_set.append(tuple(opt_state_list))
    params = flax.core.unfreeze(params)
    params['/dense-1_layernorm_tanh'] = params['']['dense-1_layernorm_tanh']
    params.pop('')
    return flax.core.freeze(params), new_opt_state_set
  ...
  def create_masks(self, param_dict, activations_dict, key):
    ...
    # prepare mask of incoming and outgoing recycled connections
    # print(self.reset_layers)['conv0', 'conv1', 'conv2', 'conv3', 'dense-1_layernorm_tanh', 'critic0/dense0', 'critic0/dense1', 'critic1/dense0']
    next_layers = self.next_layers_for_enc
    critic_count = 0
    for k in self.reset_layers:
      if 'layernorm' in k:
        if critic_count == 0:
          next_layers = self.next_layers_for_critic0
          critic_count += 1
      if 'critic0/dense1' in k:
        k = self.reset_layers[4]
        next_layers = self.next_layers_for_critic1
      next_key = next_layers[k]
      prefix, prefix_next = prefix_creator(k), prefix_creator(next_key)

      param_key = prefix + k + '/kernel' # TODO needs to be specified for each algo (if using different network architectures)
      param = param_dict[param_key]
      # This won't work for DRQ, since returned keys can be a list.
      # We don't support that at the moment.
      next_param = param_dict[prefix_next + next_key + '/kernel']
      activation = activations_dict[prefix + k + '_act/__call__'][0]
      neuron_mask = self._score2mask(activation, param, next_param, key)

      # the for loop handles the case where a layer has multiple next layers
      # like the case in DrQ where the output layer has multihead.
      next_param_key = prefix_next + next_key + '/kernel'
      next_param = param_dict[next_param_key]
      incoming_mask, outgoing_mask = self.create_mask_helper(
          neuron_mask, param, next_param
      )
      if outgoing_mask.shape[0] != next_param.shape[0]:
        outgoing_mask = jnp.vstack([
          outgoing_mask,
          jnp.zeros((next_param.shape[0] - outgoing_mask.shape[0], 256))
      ])
      incoming_mask_dict[param_key] = incoming_mask
      outgoing_mask_dict[next_param_key] = outgoing_mask
      key, subkey = random.split(key)
